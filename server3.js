...
// Ensure index.html exists so the root route doesn't throw a 500 error
...
const server = http.createServer((req, res) => {
    ...
    if (method === 'GET' && url === '/exit') {
        myEmitter.emit('exit');
        sendJSON(res, 200, { message: 'Exit event emitted. Check the server console!' });
        
        // Shut down the server gracefully after sending the response
    ...
    if (method === 'POST' && url === '/create') {
        ...
        req.on('end', () => {
            try {
                // Safeguard against invalid JSON crashing the server
